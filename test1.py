...
# 2. 테이블을 분리
# 북아이디 별로의 정보 = 출판사, 책 제목, 평점=평균
# 유저 아이디별로의 정보 = 우저 정보 주소 나이
...
# 북 아이디 기준으로 분류할 시 같은 책이 여러개 나열 될 것이다.
# 그 중에서 뺄 것을 빼야 한다.
# 북 아이디 기준으로 정렬시 같은 책 - 더티 데이터가 포함된 출판사 이름, 책 제목이 나올 것이다.
# 이것을 
# 필na를 사용한다?


# 북 아이디가 동일한 것끼리 뭉친 뒤, 그 안에서 더티 데이터를 드롭, fillna 같은 것으로 바꾼다.
